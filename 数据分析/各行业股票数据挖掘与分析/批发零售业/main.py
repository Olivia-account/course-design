...
# 检查表是否存在，如果存在则删除
drop_table_query = 'DROP TABLE IF EXISTS ' + baseName
...
# 插入数据的 SQL 语句
insert_query = '''
INSERT INTO ''' + baseName +''' (industry, company, title, source, href, date, score, closeprice)
...
# 保存工作簿到文件
workbook.save(baseName+'data.xlsx')
...
# 保存散点图为 JPG 文件
plt.savefig(baseName+'scatter_plot_PFLSY.jpg')
plt.show()